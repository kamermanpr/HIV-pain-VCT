...
tmp <- data %>% 
    select(HIV_positive, pain_in_last_month, pain_reason_for_visit, pain_worst, pain_treatment,
           ...
           legs, feet, joints, muscles, starts_with('medication'), starts_with('substance'))
...
tmp %>% 
    ...
    arrange(pct_missing) %>% 
    kable(., caption = 'Missingness (whole cohort)')
...
tmp %>% 
    ...
    mutate(pct_missing = round(100 * pct_missing)) %>% 
    arrange(pct_missing) %>% 
    kable(., caption = 'Missingness (HIV-negative)')
...
tmp %>% 
    ...
    mutate(pct_missing = round(100 * pct_missing)) %>% 
    arrange(pct_missing) %>% 
    kable(., caption = 'Missingness (HIV-positive)')
...
## Medications/substances used for pain management

### Process data

```{r pain_drugs}
drugs <- data %>% 
    select(PID, HIV_positive, 
           starts_with('medication'), 
           starts_with('substance')) %>% 
    unite(col = 'drugs', 3:8, sep = ' + ') %>% 
    # Search terms based on visual inspection of the data
    mutate(paracetamol = case_when(
        str_detect(drugs, 'paracetamol') ~ 'yes',
        TRUE ~ 'no'
    )) %>% 
    mutate(nsaids_aspirin = case_when(
        str_detect(drugs, 'aspirin') |
            str_detect(drugs, 'ibuprofen') |
            str_detect(drugs, 'diclofenac') ~ 'yes',
        TRUE ~ 'no'
    )) %>% 
    mutate(weak_opioids = case_when(
        str_detect(drugs, 'tramadol') |
            str_detect(drugs, 'codeine')  ~ 'yes',
        TRUE ~ 'no'
    )) %>% 
    mutate(marijuana = case_when(
        str_detect(drugs, 'marijuana') ~ 'yes',
        TRUE ~ 'no'
    )) %>% 
    mutate(alcohol = case_when(
        str_detect(drugs, 'alcohol') ~ 'yes',
        TRUE ~ 'no'
    )) %>% 
    select(-drugs) %>% 
    gather(key = drug,
...
# Total cohort
drugs %>% 
    group_by(drug, drug_used) %>% 
    summarise(count = n()) %>% 
    group_by(drug) %>% 
    mutate(total = sum(count)) %>% 
    filter(drug_used == 'yes') %>% 
    mutate(proportion = round(count/total, 2)) %>% 
    select(-total) %>% 
    kable(caption = 'Drugs used for pain - total cohort')
...
ggplot(data = drugs) +
    ...
    scale_fill_manual(values = pal, 
                      na.value = '#000000') +
    labs(title = 'Drugs used to manage pain',
         subtitle = 'Whole cohort',
         y = 'Proportion',
         x = 'Drug class') +
    theme(legend.title = element_blank(),
...
# By HIV status
drugs %>% 
    group_by(HIV_positive, drug, drug_used) %>% 
    summarise(count = n()) %>% 
    group_by(drug) %>% 
    mutate(total = sum(count)) %>% 
    filter(drug_used == 'yes') %>% 
...
func_tmp <- function(d, i){
    data <- d[i, ]
    data <- data %>% 
        filter(!is.na(drug_used))
    prop <- mean(data$drug_used == 'yes')
    prop
}
...
tibble(`drug class` = boot_tmp$drug,
       proportion = c(round(boot_tmp$boot[[1]]$t0, 2), 
                      round(boot_tmp$boot[[2]]$t0, 2),
                      round(boot_tmp$boot[[3]]$t0, 2), 
                      round(boot_tmp$boot[[4]]$t0, 2),
       ...
       `upper 95% CI` = c(round(boot_tmp$boot_ci[[1]]$percent[[5]], 2), 
                          ...
                          round(boot_tmp$boot_ci[[4]]$percent[[5]], 2),
                          round(boot_tmp$boot_ci[[5]]$percent[[5]], 2))) %>% 
    kable(caption = 'Drug classes used to manage pain - whole cohort (95% CI)')

# By HIV status (HIV- reported first)
set.seed(2019)
boot_tmp <- drugs %>% 
    group_by(HIV_positive, drug) %>% 
    nest() %>% 
    mutate(boot = map(.x = data,
                      ~ boot(data = .x, 
                             statistic = func_tmp, 
...
tibble(`HIV positive` = rep(c('no', 'yes'), 5),
       `drug class` = boot_tmp$drug,
       proportion = c(round(boot_tmp$boot[[1]]$t0, 2), 
                      round(boot_tmp$boot[[2]]$t0, 2),
                      round(boot_tmp$boot[[3]]$t0, 2), 
                      round(boot_tmp$boot[[4]]$t0, 2),
...
The difference in the proportion of HIV-positive participants using a drug vs participants who were HIV-negative (HIV+ minus HIV-).
...
func_tmp <- function(d, i){
    ...
    data_hiv <- filter(data, HIV_positive == 'yes')
    data_nohiv <- filter(data, HIV_positive == 'no')
    prop_yes <- mean(data_hiv$drug_used == 'yes')
    prop_no <- mean(data_nohiv$drug_used == 'yes')
    prop_yes - prop_no
}
...
tibble_tmp <- tibble(`drug class` = boot_tmp$drug,
       `difference in proportion` = c(round(boot_tmp$boot[[1]]$t0, 2), 
                                      round(boot_tmp$boot[[2]]$t0, 2),
                                      round(boot_tmp$boot[[3]]$t0, 2), 
                                      round(boot_tmp$boot[[4]]$t0, 2),
...
tibble_tmp %>% 
    kable(caption = 'Drugs used to manage pain - 95% CI of the difference (HIV+ minus HIV-)')
